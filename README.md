...
### 渗流阈值的 Monte Carlo 估计

在不同晶格尺寸上分别进行 1000 次独立实验。随着晶格尺寸增大，阈值分布更加集中，估计结果稳定在理论临界值附近。

![不同晶格尺寸下的渗流阈值分布](images/PartI/sta.png)

### 不同逆温度下的 Ising 自旋构型

黑白像素分别表示两种自旋状态。系统从高温下的无序状态逐渐形成大尺度自旋团簇，并在低温区域表现出明显的有序化现象。

![不同逆温度下的二维 Ising 模型自旋构型](images/PartII/TaskB/Figure_.png)

### 磁化强度与能量的相变特征

平均绝对磁化强度和平均单自旋能量随逆温度发生显著变化；红色虚线标出了二维 Ising 模型的理论临界逆温度 `β ≈ 0.441`。

![磁化强度和能量随逆温度的变化](images/PartII/TaskB/Figure1.png)
